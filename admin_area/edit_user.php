    ...
    $email=$_SESSION['admin_email'];
        if (isset($_GET['edit_worker'])) {
            ...
            if (isset($_POST['insert'])) {
               ...
               $admin_contact=strip_tags($_POST['admin_contact']);
               $admin_job=strip_tags($_POST['admin_job']);
               $admin_password=$worker_password;
               $admin_image= $_FILES['image_1']['name'];
               $admin_country= strip_tags($_POST['admin_country']);
               $admin_desc= strip_tags($_POST['admin_desc']);
                ...
                if (isset($_FILES['image_1']['name'])) {
                  ...
                  if ($admin_job=="Admin") {
                    $sql5 ="SELECT *FROM admin WHERE admin_email='$admin_email' OR admin_password='$admin_password'";
                    $res5=mysqli_query($con, $sql5);
                    $Verify=mysqli_num_rows($res5);
                            if ($Verify==1) {
                                echo "<script>alert('email já está sendo utilizado')</script>";
                                echo "<script>window.open('index.php?edit_worker=$edit_user','_self')</script>"; 
                    ...
                    $res4=mysqli_query($con, $sql4);
                       ...
                       //Redirect to manage category page
                           ...
                           echo "<script>alert('falhou em cadastrar o admin')</script>";
                    ...
                    if($res2==true){
                       ...
                       //Redirect to manage category page
                           ...
                           //echo("Desculpe, verifique a query");
                           //Create a ssession variable to display message
                           echo "<script>alert('falhou em actualizar o $admin_job')</script>";
                  ...
                  if ($admin_job=="Admin") {
                    ...
                    $Verify=mysqli_num_rows($res5);
                            if ($Verify==1) {
                        echo "<script>alert('Email ou senha já estão a se utilizados')</script>";
                  ...
                  if ($admin_job=="Admin") {
                    ...
                    if($res3==true){
                       ...
                       //Redirect to manage category page
                           ...
                           //echo("Desculpe, verifique a query");
                           //Create a ssession variable to display message
                           echo "<script>alert('falhou em actualizar o admin')</script>";
                          
                   }
                  } if((($email!=$worker_email)AND ($admin_job=="Funcionário")) OR (($email!=$worker_email) AND($admin_job=="Gerente"))) {
                    $sql5 ="SELECT *FROM worker WHERE worker_email='$admin_email' OR worker_password='$admin_password'";
                    $res5=mysqli_query($con, $sql5);
                    $Verify=mysqli_num_rows($res5);
                    if ($Verify==1) {
                        echo "<script>alert('Email ou senha já estão a se utilizados')</script>";
            ...
            $res4=mysqli_query($con, $sql4);
                ...
                if($res2==true){
                   ...
                   //Redirect to manage category page
                       ...
                       //echo("Desculpe, verifique a query");
                       //Create a ssession variable to display message
                       echo "<script>alert('falhou em cadastrar o $admin_job')</script>";
                      
               }
                    }
                  }
                  else{
                    echo "<script>alert('Operação impossivel de ser realizada')</script>";
                    echo "<script>window.open('index.php?edit_user','_self')</script>";
                    die();
                  }
                    //redirect to add category page
