   ...
   $admin_desc= $_POST['admin_desc'];
    ...
    if (isset($_FILES['image_1']['name'])) {
       ...
       $loadup =move_uploaded_file($crs, $tsd);
        ...
        if ($loadup == false) {
        //set message
        echo "<script>alert('falhou ao carregar a imagem')</script>";
      ...
      if ($admin_job=="Admin") {
        $sql4 ="SELECT *FROM admin WHERE admin_email='$admin_email' OR admin_password='$admin_password'";
        $res4=mysqli_query($con, $sql4);
        $Verify=mysqli_num_rows($res4);
       if ($Verify==1) {
        echo "<script>alert('Email ou Senha já estão a ser utlizados')</script>";
        ...
        echo "<script>alert('Falhou ao adicionar um novo Administrador')</script>";
        echo "<script>window.open('index.php?insert_user','_self')</script>";
        die();
        ...
        $sql6 ="SELECT *FROM worker WHERE worker_email='$admin_email' OR worker_password='$admin_password'";
        $res6=mysqli_query($con, $sql6);
        $Verify=mysqli_num_rows($res6);
        if ($Verify==1) {
        echo "<script>alert('Email ou Senha já estão a ser utlizados')</script>";
           echo "<script>window.open('index.php?insert_user','_self')</script>";
           die();
        ...
        if($res2==true){
           ...
           //Redirect to manage category page
               ...
               echo "<script>alert('Falhou em adicionar novo $admin_job')</script>";
