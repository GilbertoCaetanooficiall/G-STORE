<?php if (!isset($_SESSION['worker_email'])) {
    echo "<script>window.open('login.php','_self')</script>";
        ...
        if (isset($_GET['edit_profile'])) {
            ...
            $select_admin="SELECT * FROM worker WHERE worker_id='$edit_user'";
            $run_select_admin=mysqli_query($con,$select_admin);
            $fetch_admin=mysqli_fetch_assoc($run_select_admin);
            $worker_name= $fetch_admin['worker_name'];
            $worker_email= $fetch_admin['worker_email'];
            $worker_contact= $fetch_admin['worker_contact'];
            $worker_job= $fetch_admin['worker_job'];
            $worker_image= $fetch_admin['worker_image'];
            $worker_country= $fetch_admin['worker_country'];
            $worker_desc= $fetch_admin['worker_desc'];
            $uma=$worker_email;
